...
def load_config_from_gcs(gcs_uri: str) -> dict:
  """Downloads a YAML configuration file from GCS and parses it."""
...
if __name__ == "__main__":
  CONFIG_GCS_URI = "gs://aspect-application-poc-bucket/profiler_config.yaml"

  try:
    logger.info(f"Starting execution. Reading config from {CONFIG_GCS_URI}...")
    config = load_config_from_gcs(CONFIG_GCS_URI)

    project_id = config.get("project_id")
    dataset_id = config.get("dataset_id")
    table_id = config.get("table_id")
    location = config.get("location", "us-central1")
    exclude_strings = config.get("exclude_strings", False)

    if not project_id or not dataset_id or not table_id:
      logger.error(
          "Config validation failed. 'project_id', 'dataset_id', and 'table_id'"
          " are required in the YAML file."
      )
      sys.exit(1)

    logger.info(
        f"Target target loaded successfully: {project_id}.{dataset_id}.{table_id}"
    )

    profiler = DataProfiler(project_id, location)
    profiler.profile_table(
        dataset_id, table_id, exclude_strings=exclude_strings
    )

    logger.info("Cloud Run Job task completed successfully.")
    sys.exit(0)
